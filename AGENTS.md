...
All repository documentation must be created and maintained in English.

This requirement applies to:
- `README.md` and other Markdown documentation
- all documents under `docs/`
- human-readable descriptions, summaries, examples, and comments in documentation-oriented YAML, JSON, or similar files

When updating an existing document, write all new or revised prose in English. If the document is not already in English, translate the affected document to English as part of the same change so that it does not become mixed-language.

Source quotations, user-provided content, proper names, code identifiers, CLI output, and localization examples may remain in their original language when preserving them is necessary for accuracy.
