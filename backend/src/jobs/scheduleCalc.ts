  ...
  const candidatesResult = await db
    ...
    .query(
      [
        ...
        "  CAST(",
        "    COALESCE(s.NextPMDueAt, dateadd(day, t.IntervalDays, COALESCE(h.LastCompletedAt, s.LastPMCompletedAt, sysutcdatetime())))",
        "    AS datetime2(0)",
        "  ) AS NextDueAt,",
        ...
        "  AND t.IsActive = 1",
        "  AND COALESCE(s.NextPMDueAt, dateadd(day, t.IntervalDays, COALESCE(h.LastCompletedAt, s.LastPMCompletedAt, sysutcdatetime()))) <= dateadd(day, @horizonDays, sysutcdatetime())",
      ].join("\n"),
    );
