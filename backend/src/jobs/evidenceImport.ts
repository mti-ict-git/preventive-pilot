      ...
      await tx
        ...
        .query(
          [
            ...
            "  END,",
            "  NextPMDueAt = CASE",
            "    WHEN s.DefaultTemplateId = @templateId AND t.IntervalDays > 0 THEN CASE",
            "      WHEN t.IntervalDays = 30 THEN dateadd(month, 1, @completedAt)",
            "      WHEN t.IntervalDays = 90 THEN dateadd(month, 3, @completedAt)",
            "      WHEN t.IntervalDays = 180 THEN dateadd(month, 6, @completedAt)",
            "      WHEN t.IntervalDays = 365 THEN dateadd(year, 1, @completedAt)",
            "      ELSE dateadd(day, t.IntervalDays, @completedAt)",
            "    END",
            "    ELSE s.NextPMDueAt",
            "  END,",
