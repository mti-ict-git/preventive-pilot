  ...
  const result = await db
    ...
    .query(
      [
        ...
        "  ) nextDue",
        "  OUTER APPLY (",
        "    SELECT MAX(bw.EndsAt) AS MaxEndsAt",
        "    FROM pm.BlackoutWindows bw",
        "    WHERE bw.IsActive = 1",
        "      AND bw.StartsAt <= nextDue.CandidateDueAt",
        "      AND bw.EndsAt >= nextDue.CandidateDueAt",
        "  ) blackout",
        "  OUTER APPLY (",
        "    SELECT CAST(COALESCE(blackout.MaxEndsAt, nextDue.CandidateDueAt) AS datetime2(0)) AS DueAt",
        "  ) due",
        "  WHERE a.IsArchived = 0",
        "    AND s.PMEnabled = 1",
