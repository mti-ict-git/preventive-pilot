...
Q-21: User confirmed technician repair-completion submission followed by Supervisor verification and WO closure, with downtime ending when equipment is restored even if the WO remains open administratively. Implementation is pending under [CM-01](implementation-roadmap.md#cm-01). Also confirmed: return incomplete repair on the same WO with a mandatory written reason and retained history/evidence; a single review by Supervisor/Admin/Superadmin; no repair performer may verify their own WO regardless of role. Do not reopen these product decisions. Also confirmed: the assigned technician or Supervisor/Admin/Superadmin may record restoration independently of verification; default to now, with actual past restoration time requiring a written reason and change history. Same-fault recurrence before closure appends an outage interval to the same WO; after closure it creates a new linked WO. Operational gaps are excluded from downtime. Remaining boundaries: correction/resubmission access, performer attribution across handoffs, timestamp validation/event correction, interval storage and concurrency, duplicate/link handling, and existing-record migration. Do not treat recurrence after closure as reopening the closed WO. Inspect before asking dependent questions. Q-09 owns metric definitions; do not equate administrative closure duration with equipment downtime.

## Code-audit clarification register — 2026-09-11

The [technical map](code-implementation-map.md) supplies evidence and bounded work under existing questions; it does not reopen approved product decisions. Q-01/Q-02 include direct-submit validation, missing-item checks and shared PM/CM mutation bypasses. Q-13 includes sync/generator race protection and affected cancellation states. Q-16/Q-20 include legacy snapshot/import/delete behavior and replacement period uniqueness. Q-19/Q-21 include performer history across assignment/account deletion and CM self-verification. Q-04/Q-18 include missing PM-only history predicates and asset/facility recurrence parity. Q-09 includes the observed asset-only INNER JOIN in compliance and reported-to-completed CM metric, which must not be represented as multi-interval downtime. These are static findings requiring implementation and runtime verification, not proven live incidents.
