...
## Sat Feb 14 15:35:00 WITA 2026
- Backend: logged task cancel/reopen actions to preserve remark history across reopen.
- Backend: exposed audit-backed remarksHistory in task detail response.
- Web: remarks history now hides empty notes (no more “—” placeholders).
- Verified with `npm run lint` and `npm --prefix backend run typecheck`.
